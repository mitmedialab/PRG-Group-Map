...
export type GraphEntity = "node" | "edge";
export type Class = "title" | "director" | "project" | "theme" | "staff" | "department" | "person";
...
const getStyle = <T extends (Record<keyof NodeSpecifier | keyof EdgeSpecifier, string> | "all")>(entity: GraphEntity, specifier: T, details: StyleWithoutSelector, modifier?: string): cytoscape.Stylesheet => ({
    selector: getSelector(
        entity,
        isString(specifier) ? "" : getSpecifier(Object.keys(specifier)[0], Object.values(specifier)[0]),
        modifier),
    ...details
});

type Element = cytoscape.ElementDefinition;
export const node = (data: Element['data'] & { class: Class }): Element => ({ data, group: "nodes" });
export const edge = (data: Element['data']): Element => ({ data, group: "edges" });

type NodeSpecifier = { class: Class } | { theme: string } | { id: string } | "all";
export const nodeStyle = (specifier: NodeSpecifier, details: StyleWithoutSelector, modifier?: string) => getStyle("node", specifier, details, modifier);

type EdgeSpecifier = { source: string } | { target: string } | "all";
export const edgeStyle = (specifier: EdgeSpecifier, details: StyleWithoutSelector, modifier?: string) => getStyle("edge", specifier, details, modifier);
...
type NodesToCollect = NodeSingular | NodeCollection | CollectionReturnValue;
export const collect = ({ include, exclude }: { include: NodesToCollect, exclude: NodesToCollect[] }) =>
    exclude.reduce((collection, exclusion) => {
        return collection.difference(exclusion);
    }, include);